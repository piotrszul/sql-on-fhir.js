...
`generation_error`, `execution_error`, `timeout`, `malformed`). HOW a runner makes
partial results durable (for example an append-per-cell native log) is an
IMPLEMENTATION technique that this contract does NOT mandate; the contract requires
only that whatever cases completed are recorded and validate.
