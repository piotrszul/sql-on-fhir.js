...
- [x] 1.1 Add `endTime: 20250101` to `benchmark/clinical-flat.json` recipe
  `params` (see `design.md` Decision 2 for the value + rationale — pending human
  gate approval)
- [x] 1.2 Add the moved export toggles to `params` at their current-behaviour
...
- [x] 1.3 Confirm `benchmark/benchmark.schema.json` accepts the new `params`
...
- [x] 2.1 Write a failing unit test for `makeSyntheaExecutor` that captures the
  ...
  flag hardcoded
- [x] 2.2 Write a failing unit test asserting that a `synthea` recipe with
  `params.endTime` omitted is rejected by the benchmark invariant validator
- [x] 2.3 Run the new tests with `bun test` and CONFIRM they fail for the right
...
- [x] 3.1 In `benchmark/tools/executors/synthea.js`, add `-e String(p.endTime)`
...
- [x] 3.2 Add `--generate.thread_count=1` to the args
- [x] 3.3 Replace the hardcoded `--exporter.hospital.fhir.export=false`,
  `--exporter.practitioner.fhir.export=false`, `--exporter.fhir.bulk_data=true`
...
- [x] 3.4 Add the `synthea`-recipe `endTime`-required check to the benchmark
  invariant validator
- [x] 3.5 Run `bun test` and CONFIRM the tests from section 2 now pass (green
  step)

## 4. Re-bless once against the pinned end date

- [x] 4.1 With a configured `tools/executors.config.json` (Synthea 3.2.0 jar),
  materialize the recipe at sizes `s` and `m` with `force`, letting the new
  `endTime`-pinned executor run
- [x] 4.2 Re-bless `expectCount` for every case at every size via the reference
...
- [x] 4.3 Confirm the content hash / `data/<name>_<hash>/` directory changed
  because `params` changed, and the new `manifest.json` counts match the
  re-blessed `expectCount`
- [x] 4.4 Add/confirm a determinism check: materializing twice (or simulating
  two wall-clock dates by fixing the clock) yields identical manifest counts

## 5. Verify green before merge

- [x] 5.1 Run `bun test` — all pass
- [x] 5.2 Run `bun run validate` — all `tests/*.json` valid against
  `tests.schema.json` (unaffected, but must stay green)
- [x] 5.3 Run `bun run check-fmt` — clean
- [x] 5.4 Run `openspec validate pin-synthea-end-date --strict` — valid
