...
- [x] 0.1 Human confirms the #22 clarifications as in `design.md`: (D-A)
...
- [x] 0.2 Human confirms NO schema change, NO reference-runner code change, and NO
  re-bless are in scope (spec + README only)

## 1. README data-layout doc fix (the only file edit; #22.5, D-E)

- [x] 1.1 In `benchmark/README.md`, update the stale output line from
...
- [x] 2.1 Confirm `benchmark/benchmark-report.schema.json` is UNCHANGED by this
...
- [x] 2.2 Confirm the reference runner is UNCHANGED — it already records every
  non-`ok` failure as `execution_error` and its case filter is already optional; no
  runner code edit is made.
- [x] 2.3 Confirm the existing benchmark/runner tests remain green (nothing they
  exercise changed): the same statuses validate, the same runner behaviour holds.

## 3. No re-bless required (explicit no-op on data)

- [x] 3.1 This change touches NO materialized bytes and NO contract-of-data: the
...
- [x] 4.1 `bun test` — benchmark + runner tests pass (any pre-existing unrelated
  failures noted, not introduced by this change).
- [x] 4.2 `bun run validate` — all `tests/*.json` valid against `tests.schema.json`
  (unaffected, stays green).
- [x] 4.3 `bun run check-fmt` — clean.
- [x] 4.4 `openspec validate benchmark-failure-status-clarifications --strict` —
  valid.
