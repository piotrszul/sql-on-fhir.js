...
- [x] 1.2b Create `benchmark/staging-hooks/flatquack/flatquack-hook.js` — thin
      ...
      directory-glob CLI, launch flatquack under node (aehrc/flatquack#42 fix),
      and require a clean exit + written CSV (turns #43's silent failure honest)
...
- [x] 2.3 Full harness run, size `m`, `end_to_end`, report written; checkfile
      row counts verified for both cases (48908 / 39479, zero flakes across 4
      runs). Unblocked by the upstream #42 fix (flatquack under node); timing
      now trustworthy — see FINDINGS.md entry 3.
