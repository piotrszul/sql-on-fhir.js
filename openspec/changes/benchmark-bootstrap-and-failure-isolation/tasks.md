...
- [ ] 1.1 (RED) Write failing schema-validation tests for
...
- [ ] 1.2 (RED) Confirm 1.1 fails for the right reason (enum is still the four-value
  set; `message` not yet permitted under `additionalProperties: false`)
- [ ] 1.3 (GREEN) Extend the per-case `status` enum in
...
- [ ] 2.1 (RED) Write a failing runner test: in a run where one case fails
...
- [ ] 2.2 (RED) Write a failing runner test: a run interrupted after completing only
...
- [ ] 2.3 (RED) Confirm 2.1–2.2 fail for the right reason (a failing case currently
  aborts / voids the run)
- [ ] 2.4 (GREEN) Wrap each case's timing/guard in a per-case boundary so a failure
...
- [ ] 3.1 (RED) Write failing unit tests that STUB/MOCK the network fetch (do NOT
...
- [ ] 3.2 (RED) Write a failing unit test: when `tools/executors.config.json`
  supplies a jar path, the materializer uses it and does NOT invoke the fetch
- [ ] 3.3 (RED) Confirm 3.1–3.2 fail for the right reason (no auto-fetch path yet;
  config is still a prerequisite)
- [ ] 3.4 (GREEN) Add the committed `syntheaVersion → { url, sha256 }` pinned map,
...
- [ ] 4.1 (RED) Write a failing test (stub/spy the spawn; do NOT run Java) asserting
...
- [ ] 4.2 (RED) Confirm 4.1 fails for the right reason (generator still runs in the
  repo root)
- [ ] 4.3 (GREEN) Run the generator in an isolated working directory (per D-E:
...
- [ ] 5.1 This change touches NO materialized bytes and NO contract-of-data: the
...
- [ ] 6.1 `bun test` — all pass
- [ ] 6.2 `bun run validate` — all `tests/*.json` valid against `tests.schema.json`
  (unaffected, must stay green)
- [ ] 6.3 `bun run check-fmt` — clean
- [ ] 6.4 `openspec validate benchmark-bootstrap-and-failure-isolation --strict` —
  valid
