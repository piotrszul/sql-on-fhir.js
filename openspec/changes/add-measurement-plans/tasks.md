...
- [ ] 5.3 Implement memoized flatquack SQL generation in the hook (compile on
      first `run` per case, keyed by view; worktree and DuckDB binary paths
      from hook `env`)
- [ ] 5.4 Unit-test the hook's pure parts without a DuckDB binary (sentinel
      parsing, SQL assembly, memoization, env resolution)
- [ ] 5.5 Implement the staging driver: constructs the internal plan
...
- [ ] 6.1 Materialize the target benchmark's datasets at sizes `s` and `m`
      (`bun run data`)
- [ ] 6.2 Run the driver for at least two implementation identities (two
      DuckDB CLI versions and/or two flatquack refs); all cases verified
      against checkfile counts via `count`
- [ ] 6.3 Spot-check `extract` vs `count` agreement on at least one case
      (same rows from both verbs)
- [ ] 6.4 Confirm the advisory `.timer` cross-check: `samplesMs` minus
      hook-reported execute `phasesMs` is a small, roughly constant overhead
- [ ] 6.5 Load the JMH exports of both identities into JMH Visualizer and
      confirm the overlay compares them side by side with self-describing
      impl-ids
- [ ] 6.6 Record every friction point in FINDINGS.md with exactly one
      taxonomy outcome (clean passes recorded too); implement any forced
      contract/harness/doc fix test-first with a delta spec

## 7. Docs

- [ ] 7.1 Document the plan vocabulary, the module entry point, and the
      ...
      contract changes)
- [ ] 7.2 Update `benchmark/staging-hooks/README.md` with the new
      subdirectory and its migration exit criterion

## 8. Verify and refine (validation-cycle steps 6–7)

- [ ] 8.1 Full pipeline green: `bun test`, `bun run validate`,
      `bun run check-fmt`
- [ ] 8.2 Run the `simplify` skill over the branch diff; apply accepted
      findings; re-verify
- [ ] 8.3 Run the `code-review` skill over the branch diff; apply accepted
      findings; re-verify
