...
- [ ] 0.1 Human confirms `benchmark-jmh-format` as a NEW capability (rather than a
  `benchmark-report-format` delta), per `design.md` D1
- [ ] 0.2 Human confirms the export trigger: standalone
...
- [ ] 0.3 Human confirms the JMH benchmark-name string `<benchmark.name>.<case.id>`
...
- [ ] 0.4 Human confirms the CI method: normal-approximation 95% half-width
...
- [ ] 1.1 (RED) Write a failing test: given a fixture native report with a MIX of
...
- [ ] 1.2 (RED) Write a failing test: an `ok` case with empty/absent `samplesMs`
  produces no entry; a triple with no exportable case produces NO file
- [ ] 1.3 (RED) Confirm 1.1–1.2 fail for the right reason (no projection exists yet)
- [ ] 1.4 (GREEN) Implement the pure projection's cell filter (`status == ok` AND
  usable `samplesMs`); confirm 1.1–1.2 pass

## 2. primaryMetric mapping incl. recomputed scoreError/percentiles — TDD

- [ ] 2.1 (RED) Write a failing test against a HAND-COMPUTED fixture: for an `ok`
...
- [ ] 2.2 (RED) Write a failing test asserting `primaryMetric.scoreError` equals a
  HAND-COMPUTED `1.959964 * stddev / sqrt(n)` (sample n−1 stddev) for the fixture,
  and equals `0` for a single-sample case
- [ ] 2.3 (RED) Write a failing test asserting `primaryMetric.scorePercentiles`
...
- [ ] 2.4 (RED) Confirm 2.1–2.3 fail for the right reason (mapping/recompute not
  implemented)
- [ ] 2.5 (GREEN) Implement the `primaryMetric` mapping and the pinned
...
- [ ] 3.1 (RED) Write a failing test: an `ok` case with `outputRows` `r` yields
  `secondaryMetrics.rows.score == r` with a row-count `scoreUnit`
- [ ] 3.2 (RED) Write a failing test: the JMH `benchmark` name is
  `<benchmark.name>.<case.id>`, and `size` + a stable filename-safe
  `implementation` id (derived from `engine`/`binding`/`variant`) appear in
  `params`
- [ ] 3.3 (RED) Confirm 3.1–3.2 fail for the right reason
- [ ] 3.4 (GREEN) Implement `secondaryMetrics.rows`, the benchmark-name composition,
...
- [ ] 4.1 (RED) Write a failing test: exporting a report covering two benchmarks
...
- [ ] 4.2 (RED) Confirm 4.1 fails for the right reason (no per-triple file split /
  naming yet)
- [ ] 4.3 (GREEN) Implement the per-triple grouping and the sanitized
  `<benchmark>-<size>-<impl>.jmh.json` naming; confirm 4.1 passes

## 5. Trigger plumbing (per Gate A) — TDD

- [ ] 5.1 (RED) Write a failing CLI/flag test (per the Gate-A decision): invoking
...
- [ ] 5.2 (RED) Confirm 5.1 fails for the right reason (no CLI/flag wired yet)
- [ ] 5.3 (GREEN) Wire the thin CLI (`bun run jmh <report.json> <outdir>`) and/or
  the `--jmh <dir>` runner flag over the pure function; confirm 5.1 passes

## 6. No re-bless required (explicit no-op on data)

- [ ] 6.1 The projection READS an existing conforming report and touches NO
