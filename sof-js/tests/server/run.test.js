...
describe('$run operation', () => {
  test('returns JSON results for a ViewDefinition', async () => {
    const response = await fetch('http://localhost:3002/ViewDefinition/patient_demographics/$run?format=json')
  ...
  test('CSV output includes header row by default', async () => {
    const response = await fetch('http://localhost:3002/ViewDefinition/patient_demographics/$run?format=csv')
